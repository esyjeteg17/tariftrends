	...
	try {
		...
		if (operator.value.length === 0) {
			...
			let { data } = await axios.get(
				`https://81e2e5a192b60adb.mokky.dev/${category.value}?sortBy=${filter.value}&name=*${search.value}*&price[from]=${prices.min}&price[to]=${prices.max}`
			)
			...
			let { data } = await axios.get(
				`https://81e2e5a192b60adb.mokky.dev/${category.value}?operator[]=${operator.value[0]}&operator[]=${operator.value[1]}&operator[]=${operator.value[2]}&operator[]=${operator.value[3]}&operator[]=${operator.value[4]}&operator[]=${operator.value[5]}&sortBy=${filter.value}&name=*${search.value}*&price[from]=${prices.min}&price[to]=${prices.max}`
			)
			if (showTopTarrifs.value && data.length > 3) {
				topTarrifs.value = data.slice(0, 3)
				data = data.slice(3)
...
.top-wrapper {
	animation: top-anim 2s ease-in-out;
